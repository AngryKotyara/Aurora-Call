...
function historyListItem(call) {
  const peerName=escapeHtml(call.peer_name||"Неизвестный пользователь"), initial=escapeHtml(call.peer_name?.[0]?.toUpperCase()||"?"), isVideo=call.mode==="video", isIncoming=call.direction==="incoming", direction=isIncoming?"Входящий":"Исходящий", mode=isVideo?"видео":"аудио";
  const statuses={answered:"Принят",completed:"Завершён",declined:"Отклонён",started:"Ожидание ответа"};
  const status=statuses[call.status]||"Завершён", date=formatCallDate(call.created_at), machineDate=escapeHtml(call.created_at||"");
...
function auroraWaves(){ return `<div class="xperia-flow" aria-hidden="true"></div>`; }

function mediaAccessCard(permission={status:"prompt"},compact=false){
 const content={granted:{icon:"✓",title:"Камера и микрофон готовы",text:"Доступ сохранён для следующих входов на этом устройстве.",action:"Доступ разрешён"},prompt:{icon:"◉",title:"Разрешите доступ для звонков",text:"Браузер запросит камеру и микрофон один раз и запомнит ваш выбор.",action:"Разрешить камеру и микрофон"},blocked:{icon:"!",title:"Доступ заблокирован",text:"Разрешите камеру и микрофон в настройках сайта, затем повторите проверку.",action:"Повторить запрос"},"missing-device":{icon:"!",title:"Устройства не найдены",text:"Подключите камеру и микрофон, затем повторите проверку.",action:"Проверить снова"},unsupported:{icon:"!",title:"Доступ недоступен",text:"Откройте Aurora Call в современном браузере по защищённому адресу HTTPS.",action:"Недоступно"},error:{icon:"!",title:"Не удалось проверить доступ",text:"Проверьте подключение устройств и попробуйте ещё раз.",action:"Попробовать снова"}}[permission.status];
 const isGranted=permission.status==="granted", isUnsupported=permission.status==="unsupported";
...
export function renderMain({activeScreen,session,friends,callHistory,mediaPermission,onNavigate,onSelectFriend,onCall,onGenerateInvite,onDeleteFriend,onRequestMediaAccess,onLogout}){
 const callFriends=friends.length?friends.map(friendCallCard).join(""):'<div class="card muted">Сначала добавьте друга через QR.</div>', allFriends=friends.map(friendListItem).join("")||'<div class="card muted">Друзей пока нет.</div>', historyItems=callHistory.map(historyListItem).join("")||'<div class="card empty-state"><span aria-hidden="true">◷</span><b>История пока пуста</b><p class="muted">Здесь появятся входящие и исходящие звонки.</p></div>';
 ...
 document.querySelectorAll("[data-nav]").forEach(b=>b.addEventListener("click",()=>onNavigate(b.dataset.nav)));
 document.querySelectorAll("[data-select]").forEach(card=>card.addEventListener("click",()=>onSelectFriend({id:card.dataset.select,name:card.dataset.name})));
 document.querySelectorAll("[data-call]").forEach(b=>b.addEventListener("click",e=>{e.stopPropagation();onCall(b.dataset.call,{id:b.dataset.id,name:b.dataset.name});}));
 document.querySelectorAll("[data-delete-friend]").forEach(b=>b.addEventListener("click",()=>onDeleteFriend({id:b.dataset.deleteFriend,name:b.dataset.name})));
 document.querySelectorAll("[data-request-media]").forEach(b=>b.addEventListener("click",onRequestMediaAccess));
 query("#audio")?.addEventListener("click",()=>onCall("audio")); query("#video")?.addEventListener("click",()=>onCall("video")); query("#generate-invite")?.addEventListener("click",onGenerateInvite); query("#logout")?.addEventListener("click",onLogout);
}

export function renderInvite(url){ const target=query("#invite"); if(!target)return; target.innerHTML=`<div class="invite-link">${escapeHtml(url)}</div><button id="copy-invite" class="btn ghost">Копировать ссылку</button>`; }
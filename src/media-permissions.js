...
export function isMediaAccessDisabled(storage = globalThis.localStorage) {
  try {
    return storage?.getItem(disabledStorageKey(storage)) === "1";
  } catch {
    return false;
  }
}
...
function hasStoredGrant(session, storage = globalThis.localStorage) {
  ...
  try {
    const key = deviceStorageKey(storage);
    if (storage?.getItem(key) === "granted") return true;
...
export async function inspectMediaPermissions(
  ...
  if (isMediaAccessDisabled(storage))
    return { status: "prompt", deviceId, manuallyDisabled: true };

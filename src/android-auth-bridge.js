...
function shell() {
  return document.querySelector(".auth-v2");
}
...
function showRegistration() {
  ...
  const signup = root.querySelector(".auth-v2-signup");
  if (signup) signup.style.display = "none";
  const divider = root.querySelector(".auth-v2-divider");
  if (divider) divider.style.display = "none";
  const socials = root.querySelector(".auth-v2-socials");
...
function showLogin() {
  ...
  const panel = root.querySelector("[data-auth-login]");
...
function nextRegistrationStep() {
  ...
  if (!username) {
    root.querySelector("#register-name")?.focus();
    return;
  }
  ...
  (event) => {
    ...
    if (target.id === "login") {
      ...
      document.dispatchEvent(
        new CustomEvent("aurora-auth-login", {
          detail: {
            username: root?.querySelector("#login-name")?.value || "",
            accessKey: root?.querySelector("#access")?.value || "",
          },
        }),
      );
      return;
    ...
    if (target.id === "create") {
      ...
      const root = shell();
      document.dispatchEvent(
        new CustomEvent("aurora-auth-register", {
          detail: {
            username: root?.querySelector("#register-name")?.value || "",

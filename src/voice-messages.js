...
function svgMic() { return `<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="8" y="3" width="8" height="12" rx="4"/><path d="M5 11a7 7 0 0 0 14 0M12 18v3M9 21h6"/></svg>`; }
...
function svgCheck() { return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12.5 4.2 4.2L19 7"/></svg>`; }
...
function showFinishAnimation(discarded = false) {
  ...
  composer.insertAdjacentHTML("beforebegin", `<div class="voice-finish-flash ${discarded ? "discarded" : "sent"}"><span>${discarded ? svgTrash() : svgCheck()}</span><b>${discarded ? "Запись отменена" : "Запись завершена"}</b></div>`);
  window.setTimeout(() => document.querySelector(".voice-finish-flash")?.remove(), 650);
}
...
async function beginRecording(event) {
  ...
  try {
    ...
    if (!pointerHeld && pendingRelease) { cleanupStream(); pendingRelease = false; return; }
    ...
    showRecordingUI();
    if (!pointerHeld || pendingRelease || performance.now() - requestedAt > 1200 && !pointerHeld) {
...
function trackGesture(event) {
  ...
  if (dx < -85) { cancelled = true; pointerHeld = false; stopRecording(true); }
  else if (dy < -75) { locked = true; navigator.vibrate?.(16); showLockedControls(); }
}
function stopRecording(discard = false) {
  ...
  try { recorder.stop(); } catch (error) { stopping = false; console.warn("Voice stop failed", error); }
}
...
function waveBars(seed) {
  ...
  return Array.from({ length: 34 }, (_, i) => `<i style="--h:${(5 + Math.abs(Math.sin((hash + i * 19) * .17)) * 21).toFixed(1)}px"></i>`).join("");
}
function optimisticVoice(duration) {
  ...
  bubble.className = "chat-bubble outgoing voice-message voice-finishing is-sending";
...
async function finalizeRecording() {
  ...
  if (wasCancelled || duration < .35 || !chunks.length) {
    chunks = [];
  ...
  try {
    ...
    const meta = optimistic?.querySelector(".chat-message-meta"); if (meta) meta.textContent = "Отправлено ✓✓";
...
async function resolveVoice(messageId) {
  ...
  const binary = atob(row.media_base64), bytes = new Uint8Array(binary.length);
  for (let i = 0; i < binary.length; i += 1) bytes[i] = binary.charCodeAt(i);
  const url = URL.createObjectURL(new Blob([bytes], { type: row.media_mime || "audio/webm" }));
  voiceUrls.set(messageId, url); return url;
}
async function hydrateVoiceBubble(bubble) {
  if (bubble.dataset.voiceReady) return;
  bubble.dataset.voiceReady = "loading";
  const messageId = bubble.dataset.messageId, durationMs = Number(bubble.dataset.voiceDuration || 0), meta = bubble.querySelector(".chat-message-meta");
  ...
  try {
    const src = await resolveVoice(messageId), audio = new Audio(src); audio.preload = "metadata";
    const button = player.querySelector(".voice-play"), wave = player.querySelector(".voice-wave"), durationLabel = player.querySelector(".voice-duration");
    ...
    audio.ontimeupdate = () => { const percent = audio.duration ? Math.min(100, audio.currentTime / audio.duration * 100) : 0; wave.style.setProperty("--voice-progress", `${percent}%`); wave.setAttribute("aria-valuenow", String(Math.round(percent))); durationLabel.textContent = formatDuration(audio.currentTime || audio.duration || durationMs / 1000); };
    ...
    button.onclick = async () => { document.querySelectorAll("audio[data-aurora-voice]").forEach((other) => { if (other !== audio) other.pause(); }); if (audio.paused) { await audio.play(); button.innerHTML = svgPause(); } else { audio.pause(); button.innerHTML = svgPlay(); } };
    audio.dataset.auroraVoice = messageId; bubble.append(audio); audio.hidden = true; bubble.dataset.voiceReady = "true";
  } catch (error) { console.error("Voice hydrate failed", error); bubble.dataset.voiceReady = "error"; player.classList.add("voice-error"); }
}
function decorateVoiceBubbles() {
  document.querySelectorAll('#chat-layer .chat-bubble[data-message-kind="audio"]').forEach((bubble) => {
    if (!bubble.dataset.voiceDuration) { const body = bubble.querySelector(".chat-message-text"); if (body && /^\d+$/.test(body.textContent.trim())) { bubble.dataset.voiceDuration = body.textContent.trim(); body.remove(); } }
    bubble.classList.add("voice-message"); void hydrateVoiceBubble(bubble);
  });
}
function installComposer() {
  ...
  const send = composer.querySelector(".chat-send"), input = composer.querySelector(".chat-input");
  if (!send || !input) return;
  const mic = document.createElement("button"); mic.type = "button"; mic.className = "chat-voice-record"; mic.setAttribute("aria-label", "Удерживайте для записи голосового сообщения"); mic.innerHTML = svgMic(); send.insertAdjacentElement("afterend", mic);
  const sync = () => { const hasText = Boolean(input.value.trim()); mic.hidden = hasText; send.hidden = !hasText; }; input.addEventListener("input", sync); sync();
  mic.addEventListener("pointerdown", (event) => { event.preventDefault(); pointerHeld = true; pendingRelease = false; mic.setPointerCapture?.(event.pointerId); void beginRecording(event); });
  mic.addEventListener("pointermove", trackGesture);
  mic.addEventListener("pointerup", () => { pointerHeld = false; if (recorder && !locked) stopRecording(false); else if (!recorder) pendingRelease = true; });
  mic.addEventListener("pointercancel", () => { pointerHeld = false; if (recorder && !locked) stopRecording(true); else pendingRelease = true; });
  mic.addEventListener("lostpointercapture", () => { if (pointerHeld) return; if (recorder && !locked && !stopping) stopRecording(false); });
  mic.addEventListener("contextmenu", (event) => event.preventDefault());
}
function refresh() { installComposer(); decorateVoiceBubbles(); }

...
function icon(name) {
  const paths = {
    ...
    friends: '<path d="M8 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm8-1a3.5 3.5 0 1 0 0-7M2 21a6 6 0 0 1 12 0m1-7a5 5 0 0 1 7 4.6"/>',
...
function ensureFriendMessageActions() {
  document.querySelectorAll(".card[data-select]").forEach((card) =>
    addMessageButton(card, { id: card.dataset.select, name: card.dataset.name }),
  );
...
function normalizeNavigation(nav) {
  const labels = { home: "Главная", history: "История", friends: "Друзья", settings: "Настройки" };
  ...
  if (chatButton.parentElement !== nav || chatButton.nextElementSibling !== third) nav.insertBefore(chatButton, third);
}

function ensureShell() {
  const nav = document.querySelector(".nav");
  if (nav) normalizeNavigation(nav);
  if (!document.querySelector("#chat-layer")) document.body.insertAdjacentHTML("beforeend", '<div id="chat-layer" class="chat-layer" hidden></div>');
...
function formatThreadTime(value) {
  ...
  return date.toDateString() === now.toDateString() ? formatTime(value) : new Intl.DateTimeFormat("ru", { day: "2-digit", month: "2-digit" }).format(date);
}
...
async function renderThreads() {
  const layer = document.querySelector("#chat-layer"); if (!layer) return;
  ...
  layer.querySelector(".chat-search").oninput = (event) => { const q = event.target.value.toLowerCase().trim(); layer.querySelectorAll(".chat-thread").forEach((row) => row.hidden = !row.dataset.chatName.toLowerCase().includes(q)); };
  layer.querySelectorAll("[data-chat-friend]").forEach((row) => row.onclick = () => openChat({ id: row.dataset.chatFriend, name: row.dataset.chatName }));
}

function mediaMarkup(message) {
  if (!message.media_data) return "";
  const name = escapeHtml(message.media_name || (message.kind === "image" ? "Фото" : "Видео"));
  const direct = message.media_data.startsWith("data:") || message.media_data.startsWith("http");
  const directAttr = direct ? ` data-direct-src="${escapeHtml(message.media_data)}"` : "";
  if (message.kind === "image") return `<button class="chat-media-open chat-media-frame" data-chat-media-id="${message.id}" data-media-kind="image" data-media-name="${name}"${directAttr}><div class="chat-media-skeleton"><span class="chat-media-spinner"></span><small>Загрузка фото…</small></div><img alt="${name}" loading="eager" hidden></button>`;
...
async function loadMessages() {
  ...
  try {
    const rows = await rpc("list_chat_messages", { p_token: state.session.token, p_friend: openedFriend.id, p_before: null, p_limit: 80 });
    return rows.reverse();
  } catch (error) { console.error("Failed to load chat messages", error); showToast("Не удалось загрузить сообщения"); return []; }
}
...
async function resolveMedia(messageId, directSrc) {
  ...
  if (!row?.media_base64 || !row?.media_mime) throw new Error("media_not_found");
...
async function hydrateMedia() {
  const frames = [...document.querySelectorAll("#chat-layer [data-chat-media-id]")];
  await Promise.all(frames.map(async (frame) => {
    if (frame.dataset.hydrated === "true") return;
    frame.dataset.hydrated = "loading";
    try {
      const src = await resolveMedia(frame.dataset.chatMediaId, frame.dataset.directSrc || "");
      const skeleton = frame.querySelector(".chat-media-skeleton");
      if (frame.dataset.mediaKind === "image") {
        const img = frame.querySelector("img");
        img.onload = () => { skeleton?.remove(); img.hidden = false; frame.dataset.mediaSrc = src; frame.dataset.hydrated = "true"; };
        img.onerror = () => showUnsupportedMedia(frame, src, frame.dataset.mediaName);
        img.src = src;
      } else {
        const video = frame.querySelector("video");
        video.onloadedmetadata = () => { skeleton?.remove(); video.hidden = false; frame.dataset.hydrated = "true"; };
        video.onerror = () => showUnsupportedMedia(frame, src, frame.dataset.mediaName);
        video.src = src;
        video.load();
      }
    } catch (error) {
      ...
      frame.onclick = () => { frame.dataset.hydrated = "false"; void hydrateMedia(); };
    }
  }));
  document.querySelectorAll("#chat-layer .chat-media-open[data-media-src]").forEach((button) => { button.onclick = () => showMediaViewer(button.dataset.mediaSrc); });
}
...
function startEditing(message) {
  ...
  composer.insertAdjacentHTML("beforebegin", `<div class="chat-edit-bar"><span>${icon("edit")}</span><div><b>Редактирование</b><small>${escapeHtml(message.body || "")}</small></div><button type="button" data-cancel-edit aria-label="Отменить">×</button></div>`);
  document.querySelector("[data-cancel-edit]")?.addEventListener("click", () => { editingMessage = null; input.value = ""; document.querySelector(".chat-edit-bar")?.remove(); composer.classList.remove("is-editing"); });
}
...
async function deleteMessage(message) {
  ...
  if (!message) return;
  try {
    await rpc("delete_chat_message", { p_token: state.session.token, p_message_id: Number(message.id) });
    const cached = mediaCache.get(String(message.id)) || mediaCache.get(message.id);
    if (cached?.startsWith?.("blob:")) URL.revokeObjectURL(cached);
    mediaCache.delete(String(message.id)); mediaCache.delete(message.id);
    await renderConversation();
  } catch (error) { console.error("Failed to delete message", error); showToast("Не удалось удалить сообщение"); }
}
...
function openMessageMenu(bubble, message) {
  ...
  document.body.insertAdjacentHTML("beforeend", `<div class="chat-message-menu-backdrop"></div><div class="chat-message-menu" style="top:${top}px;left:${left}px" role="menu">${canEdit ? `<button type="button" data-message-edit>${icon("edit")}<span>Редактировать</span></button>` : ""}<button type="button" class="danger" data-message-delete>${icon("trash")}<span>Удалить</span></button></div>`);
  document.querySelector(".chat-message-menu-backdrop").onclick = closeMessageMenu;
  document.querySelector("[data-message-edit]")?.addEventListener("click", () => startEditing(message));
  document.querySelector("[data-message-delete]")?.addEventListener("click", () => void deleteMessage(message));
}

function installLongPressActions() {
  document.querySelectorAll("#chat-layer .chat-bubble[data-message-own=true]").forEach((bubble) => {
    let timer = null, startX = 0, startY = 0;
    const clear = () => { if (timer) clearTimeout(timer); timer = null; };
    bubble.addEventListener("pointerdown", (event) => {
      startX = event.clientX; startY = event.clientY;
      timer = window.setTimeout(() => { navigator.vibrate?.(16); const message = currentMessages.get(String(bubble.dataset.messageId)); openMessageMenu(bubble, message); }, 420);
    });
    bubble.addEventListener("pointermove", (event) => { if (Math.hypot(event.clientX - startX, event.clientY - startY) > 8) clear(); });
    bubble.addEventListener("pointerup", clear);
    bubble.addEventListener("pointercancel", clear);
    bubble.addEventListener("contextmenu", (event) => { event.preventDefault(); const message = currentMessages.get(String(bubble.dataset.messageId)); openMessageMenu(bubble, message); });
  });
}
...
async function renderConversation() {
  ...
  closeMessageMenu(); editingMessage = null;
  const messages = await loadMessages();
  currentMessages = new Map(messages.map((message) => [String(message.id), message]));
  ...
  const messagesEl = layer.querySelector(".chat-messages"); requestAnimationFrame(() => { messagesEl.scrollTop = messagesEl.scrollHeight; });
  layer.querySelector("[data-chat-back]").onclick = () => { openedFriend = null; void renderThreads(); };
  layer.querySelector("[data-chat-attach]").onclick = () => layer.querySelector(".chat-file").click();
  layer.querySelector(".chat-file").onchange = (event) => void sendMedia(event.target.files?.[0]);
  layer.querySelector(".chat-composer").onsubmit = (event) => { event.preventDefault(); void sendText(); };
  const input = layer.querySelector(".chat-input");
  input.onkeydown = (event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendText(); } };
  input.oninput = () => { input.style.height = "auto"; input.style.height = `${Math.min(input.scrollHeight, 120)}px`; };
  layer.querySelectorAll("[data-chat-call]").forEach((button) => { button.onclick = () => { state.selectedFriend = { id: openedFriend.id, name: openedFriend.name }; document.dispatchEvent(new CustomEvent("aurora-chat-call", { detail: { mode: button.dataset.chatCall } })); }; });
  installLongPressActions(); void hydrateMedia();
}
...
async function sendText() {
  ...
  if (editingMessage) {
    ...
    try { await rpc("edit_chat_message", { p_token: state.session.token, p_message_id: Number(target.id), p_body: text }); editingMessage = null; document.querySelector(".chat-edit-bar")?.remove(); await renderConversation(); }
    catch (error) { console.error("Failed to edit message", error); showToast("Не удалось изменить сообщение"); }
    return;
  }
  input.value = ""; input.style.height = "auto";
  const messagesEl = document.querySelector(".chat-messages"); messagesEl?.querySelector(".chat-empty-conversation")?.remove();
  const optimistic = document.createElement("article"); optimistic.className = "chat-bubble outgoing is-sending"; optimistic.innerHTML = `<div class="chat-message-text">${escapeHtml(text)}</div><div class="chat-message-meta"><span>Отправка…</span></div>`; messagesEl?.append(optimistic); if (messagesEl) messagesEl.scrollTop = messagesEl.scrollHeight;
  try { await rpc("send_chat_message", { p_token: state.session.token, p_to: openedFriend.id, p_kind: "text", p_body: text, p_media_data: null, p_media_mime: null, p_media_name: null }); await renderConversation(); }
  catch (error) { optimistic.remove(); input.value = text; console.error(error); showToast("Не удалось отправить сообщение"); }
}

function readFileBase64(file, onProgress) {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onprogress = (event) => { if (event.lengthComputable) onProgress(Math.min(65, Math.round((event.loaded / event.total) * 65))); };
    reader.onload = () => { onProgress(68); resolve(String(reader.result).split(",")[1]); };
...
function createUploadBubble(file) {
  const messagesEl = document.querySelector(".chat-messages"); messagesEl?.querySelector(".chat-empty-conversation")?.remove();
  const article = document.createElement("article"); article.className = "chat-bubble outgoing chat-upload-bubble";
  const preview = URL.createObjectURL(file), isImage = file.type.startsWith("image/");
  ...
  messagesEl?.append(article); if (messagesEl) messagesEl.scrollTop = messagesEl.scrollHeight;
  return { article, preview, update(value) { const ring = article.querySelector(".chat-upload-ring"); if (ring) { ring.style.setProperty("--p", String(value)); ring.querySelector("b").textContent = `${value}%`; } } };
}

async function sendMedia(file) {
  if (!file || !openedFriend) return;
  if (!file.type.startsWith("image/") && !file.type.startsWith("video/")) return showToast("Можно отправлять фото и видео");
  if (file.size > MAX_MEDIA_BYTES) return showToast("Файл слишком большой. Максимум 1 ГБ");
  const upload = createUploadBubble(file); let fakeProgress = null;
  try {
    const base64 = await readFileBase64(file, (p) => upload.update(p));
    let p = 70; upload.update(p);
    fakeProgress = window.setInterval(() => { p = Math.min(94, p + Math.max(1, Math.round((95 - p) / 5))); upload.update(p); }, 180);
    await rpc("upload_chat_media", { p_token: state.session.token, p_to: openedFriend.id, p_kind: file.type.startsWith("image/") ? "image" : "video", p_body: null, p_media_mime: file.type, p_media_name: file.name, p_media_base64: base64 });
    upload.update(100); await new Promise((resolve) => setTimeout(resolve, 180)); await renderConversation();
  } catch (error) {
    console.error("Failed to send chat media", error); upload.article.classList.add("is-failed"); upload.article.querySelector(".chat-upload-mask small").textContent = "Ошибка отправки"; showToast("Не удалось отправить файл");
  } finally { if (fakeProgress) clearInterval(fakeProgress); URL.revokeObjectURL(upload.preview); }
}
...
function showMediaViewer(src) {
  ...
  const viewer = document.querySelector(".chat-viewer:last-of-type"); viewer.onclick = (event) => { if (event.target === viewer || event.target.closest(".chat-viewer-close")) viewer.remove(); };
}

async function refreshBadge() {
  if (!state.session) return;
  const threads = await getThreads(), total = threads.reduce((sum, thread) => sum + Number(thread.unread_count || 0), 0), badge = document.querySelector(".chat-nav-badge");
  if (badge) { badge.hidden = !total; badge.textContent = total > 99 ? "99+" : String(total); }
  const signature = threads.map((thread) => `${thread.friend_id}:${thread.last_at}:${thread.unread_count}`).join("|");
  if (lastThreadSignature && signature !== lastThreadSignature && openedFriend && !document.querySelector("#chat-layer")?.hidden) await renderConversation();

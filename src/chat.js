...
function ensureFriendMessageActions() {
  document.querySelectorAll(".card[data-select]").forEach((card) => addMessageButton(card, { id: card.dataset.select, name: card.dataset.name }));
  document.querySelectorAll(".friend-row").forEach((row) => {
    const deleteButton = row.querySelector("[data-delete-friend]");
    if (deleteButton) addMessageButton(row, { id: deleteButton.dataset.deleteFriend, name: deleteButton.dataset.name });
  });
}
...
function closeChat() { openedFriend = null; const layer = document.querySelector("#chat-layer"); if (layer) layer.hidden = true; document.body.classList.remove("chat-active"); }
...
function mediaMarkup(message) {
  ...
  if (message.kind === "image") return `<button class="chat-media-open chat-media-frame" data-chat-media-id="${message.id}" data-media-kind="image" data-media-name="${name}"${directAttr}><div class="chat-media-skeleton"><span class="chat-media-spinner"></span><small>Загрузка фото…</small></div><img alt="${name}" loading="lazy" hidden></button>`;
...
function messageMarkup(message) {
  ...
  return `<article class="chat-bubble ${incoming ? "incoming" : "outgoing"}">${mediaMarkup(message)}${body}<div class="chat-message-meta"><time>${formatTime(message.created_at)}</time>${incoming ? "" : `<span class="chat-checks ${message.read_at ? "read" : ""}">✓✓</span>`}</div></article>`;
}
...
async function resolveMedia(messageId, directSrc) {
  ...
  const dataUrl = `data:${row.media_mime};base64,${row.media_base64}`;
  mediaCache.set(messageId, dataUrl);
  return dataUrl;
}
  ...
  await Promise.all(frames.map(async (frame) => {
    try {
      ...
      if (frame.dataset.mediaKind === "image") {
        ...
        img.onload = () => { skeleton?.remove(); img.hidden = false; frame.dataset.mediaSrc = src; };
        img.onerror = () => showUnsupportedMedia(frame, src, frame.dataset.mediaName);
        img.src = src;
      } else {
        const video = frame.querySelector("video");
        video.onloadedmetadata = () => { skeleton?.remove(); video.hidden = false; };
        video.onerror = () => showUnsupportedMedia(frame, src, frame.dataset.mediaName);
        video.src = src;
      }
    } catch (error) {
      console.error("Failed to load chat media", error);
      frame.querySelector(".chat-media-skeleton small").textContent = "Не удалось загрузить";
    }
  }));
...
async function renderConversation() {
  const layer = document.querySelector("#chat-layer"); if (!layer || !openedFriend) return;
...
async function sendText() {
  const input = document.querySelector(".chat-input"), text = input?.value.trim(); if (!text || !openedFriend) return;
...
function createUploadBubble(file) {
  ...
  const preview = URL.createObjectURL(file); const isImage = file.type.startsWith("image/");
...
async function sendMedia(file) {
  ...
  if (file.size > MAX_MEDIA_BYTES) return showToast("Файл слишком большой. Максимум 100 МБ");
  ...
  } catch (error) { console.error("Failed to send chat media", error); upload.article.classList.add("is-failed"); upload.article.querySelector(".chat-upload-mask small").textContent = "Ошибка отправки"; showToast("Не удалось отправить файл"); }

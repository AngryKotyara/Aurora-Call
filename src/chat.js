...
function normalizeNavigation(nav) {
  ...
  if (chatButton.parentElement !== nav || chatButton.nextElementSibling !== third)
...
function formatThreadTime(value) {
  ...
  return date.toDateString() === now.toDateString()
    ? formatTime(value)
    : new Intl.DateTimeFormat("ru", { day: "2-digit", month: "2-digit" }).format(
        date,
      );
}
...
function mediaMarkup(message) {
  ...
  const direct =
    message.media_data.startsWith("data:") || message.media_data.startsWith("http");
...
function openMessageMenu(bubble, message) {
  ...
  const menuWidth = 170;
  const top = Math.max(12, Math.min(window.innerHeight - 120, rect.top - 8));
  ...
  document.querySelector("[data-message-edit]")?.addEventListener("click", () =>
    startEditing(message),
  );
  document.querySelector("[data-message-delete]")?.addEventListener("click", () =>
...
async function sendMedia(file) {
  ...
  if (file.size > MAX_MEDIA_BYTES)
    return showToast("Файл слишком большой. Максимум 100 МБ");

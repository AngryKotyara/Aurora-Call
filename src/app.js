...
import { disableMediaAccess, inspectMediaPermissions, requestMediaPermissions } from "./media-permissions.js";
...
async function register(username, email) {
  try {
    if (!isValidUsername(username)) throw new Error("Имя: 3–24 буквы, цифры или _");
    ...
    const messages = { invalid_email:"Введите корректный адрес электронной почты", username_taken:"Имя пользователя уже занято", email_taken:"Эта почта уже используется", rate_limited:"Слишком много попыток. Попробуйте позже", mail_not_configured:"Отправка почты ещё не настроена на сервере", email_delivery_failed:"Не удалось отправить письмо. Попробуйте ещё раз", registration_failed:"Не удалось завершить регистрацию" };
    showToast(messages[error.message] || error.message);
  }
}
async function login(username, accessKey) {
  try {
    const session = await rpc("login_call_user", { p_username: username, p_hash: await hashSecret(accessKey) });
...
async function generateInvite(){
  try {
    const code=await rpc("create_call_friend_code",{p_token:state.session.token});
    const url=`${location.origin}/?friend=${encodeURIComponent(code)}`;
    renderInvite(url);
    query("#copy-invite").addEventListener("click",async()=>{await navigator.clipboard.writeText(url);showToast("Ссылка скопирована",true);});
    showToast("QR создан",true);
  } catch(error){showToast(error.message);}
}
async function acceptInviteFromUrl(){
  const inviteCode=new URLSearchParams(location.search).get("friend"); if(!inviteCode||!state.session)return;
  try { await rpc("accept_call_friend_code",{p_token:state.session.token,p_code:inviteCode}); history.replaceState({},"",location.pathname); showToast("Друг добавлен",true); await render("friends"); }
  catch(error){showToast(error.message);}
}
async function deleteFriend(friend){
  if(!window.confirm(`Удалить ${friend.name} из друзей?`))return;
  try { await rpc("remove_call_friend",{p_token:state.session.token,p_friend:friend.id}); if(state.selectedFriend?.id===friend.id)state.selectedFriend=null; await render("friends"); showToast(`${friend.name} удалён из друзей`,true); }
  catch(error){showToast(error.message);}
}

async function toggleMediaAccess(activeScreen){
  const current=await inspectMediaPermissions(state.session).catch(()=>({status:"prompt"}));
  ...
  const messages={granted:"Доступ к камере и микрофону разрешён",blocked:"Доступ заблокирован в настройках браузера","missing-device":"Камера или микрофон не найдены",unsupported:"Браузер не поддерживает доступ к устройствам",error:"Не удалось проверить камеру и микрофон"};
  showToast(messages[permission.status]||"Состояние доступа обновлено",permission.status==="granted");
  await render(activeScreen);
}
